@testset "Utils" begin
    ...
    @test isapprox(LinearSegmentation.heuristic_min_segment_length(xs), 1.3677823998673804; atol)
    
    @test !LinearSegmentation.is_min_length(xs[1:2], 3)
    @test LinearSegmentation.is_min_length(xs[1:2], 1)

    lmfits = LinearSegmentation.linear_segmentation([LinearSegmentation.Segment([1,2,3]), LinearSegmentation.Segment([4,5,6])], xs, ys)
